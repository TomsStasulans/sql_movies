...
const insertMovieGenres = (
  ...
  var filteredRows = genreRows.filter(genreRow => genres.includes(genreRow.genre));
  return (
    `insert into ${MOVIE_GENRES} (movie_id, genre_id) values` +
    filteredRows.map(row => `(${movieId}, ${row.id})`).join(",")
  );
};
...
const insertMovieActors = (
  ...
  var filteredRows = actorRows.filter(actorRow => actors.includes(actorRow.full_name));
  return (
...
const insertMovieDirectors = (
  ...
  var filteredRows = directorRows.filter(directorRow => directors.includes(directorRow.full_name))
  return (
...
const insertMovieKeywords = (
  ...
  var filteredRows = keywordRows.filter(keywordRow => keywords.includes(keywordRow.keyword));
  return (
...
const insertMovieProductionCompanies = (
  ...
  var filteredRows = productionCompanyRows.filter(productionCompanyRow => productionCompanies.includes(productionCompanyRow.company_name));
  return (
